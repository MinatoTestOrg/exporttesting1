...
public class ExportTesting1Table1BaseService<T extends ExportTesting1Table1Base> extends BaseBusinessLogic<T>
		implements IExportTesting1Table1BaseService<T> {
		
	private static final Logger LOGGER = LoggerFactory.getLogger(ExportTesting1Table1BaseService.class.getName());
	...
	@Autowired
	private  ExportTesting1Table1PerimeterBaseImpl exporttesting1table1PerimeterBaseImpl;
	
	
	public ExportTesting1Table1BaseService(Class<T> modelClass) {
		super(modelClass);
		...
		registerModelTransformer(PersistenceType.ANALYTICAL, new ExportTesting1Table1AnalyticalTransformer());
	}
	...
	public void onAfterSaveDB(T modelObj) {
	  changelogService.createChangeLog("ExportTesting1Table1", modelObj.getSid(), Constants.SAVED, modelObj);
	}
	...
	public void onAfterUpdateDB(T modelObj) {
	  changelogService.createChangeLog("ExportTesting1Table1", modelObj.getSid(), Constants.UPDATED, modelObj);
	}
	...
	public void onAfterDeleteDB(T modelObj) {
	  changelogService.createChangeLog("ExportTesting1Table1", modelObj.getSid(), Constants.DELETED, modelObj);
	}	
	...
	protected IPerimeterManager<T> getPerimeterManager() {
		return exporttesting1table1PerimeterBaseImpl;
	}
}