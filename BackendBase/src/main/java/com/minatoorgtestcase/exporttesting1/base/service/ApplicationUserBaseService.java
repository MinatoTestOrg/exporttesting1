...
import com.minatoorgtestcase.exporttesting1.base.service.IApplicationUserBaseService;
...
public abstract class ApplicationUserBaseService<T extends ApplicationUserBase> extends BaseBusinessLogic<T> implements IApplicationUserBaseService<T>
{
	...
	protected void setRoles(T modelObj) {
		List<String> userRoles = new ArrayList<>();
				if(BooleanUtils.isTrue(modelObj.isItachi())) {
			userRoles.add(Roles.ITACHI.getRoleName());
		}
		if(BooleanUtils.isTrue(modelObj.isMinato())) {
			userRoles.add(Roles.MINATO.getRoleName());
		}
