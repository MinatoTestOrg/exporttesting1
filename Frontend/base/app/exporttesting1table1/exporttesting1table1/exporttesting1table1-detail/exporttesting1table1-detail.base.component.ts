...
export class Exporttesting1table1DetailBaseComponent{
...
confirmationReference:any;
  ...
  "actions" : {
	...
	pageViewTitle: string = 'EXPORTTESTING1TABLE1_DETAIL';
...
this.backupData = this.appUtilBaseService.deepClone(this.detailFormControls.getRawValue());
    ...
    let action: Exclude<keyof Exporttesting1table1DetailBaseComponent, ' '> = methodName;
  ...
  const requestedObj = this.generateRequestObj(data);
      ...
      const saveSubscription = this.uploadAttachmentsandSaveData(requestedObj, splittedData,actionName).subscribe(res => {
        ...
        const saveSubscription = this.exporttesting1table1Service[method](requestedObj,actionName).subscribe((res:Exporttesting1table1Base) => {
          this.onAfterSave(res, data, method, isToastNotNeeded);
        })
        this.subscriptions.push(saveSubscription);
      }
...
uploadAttachmentsandSaveData(data: any, splittedData: any,actionName?:string): Observable<any> {
    ...
    if (!this.id) {
       const saveSubscription =  this.exporttesting1table1Service.create(splittedData.data,actionName).subscribe(
        createdData => {
    ...
    const subject$ = new Subject();    
      const updateSubscription = this.deleteFiles(splittedData).subscribe((responseData: any) => {
        this.saveFiles(responseData).subscribe((dataToUpdate: any) => {
          ...
          this.exporttesting1table1Service.update(dataToUpdate.data,actionName).subscribe(
            res => {
              subject$.next(res);
              subject$.complete();
            },
...
this.errorMessages = {
	...
	editFormula(config:any,fieldname:any){
     ...
     if(fieldConfig.formulaMethod){
       let methodName: Exclude<keyof Exporttesting1table1DetailBaseComponent, ' '> = fieldConfig.formulaMethod
    ...
    const initialState: ModalOptions = {
      initialState: {
        ...
        changelogConfig: {
          ...
          entityName : 'Exporttesting1table1',
          fieldName : null,
...
addValidations(mandatoryFields:[]){
  ...
  if (environment.prototype && this.id) {
    ...
    this.exporttesting1table1Service.getProtoTypingDataById(params).subscribe((res: any) => {
    ...
    const dataSubscription = this.exporttesting1table1Service.getById (params).subscribe((res: Exporttesting1table1Base[]) => {
      this.data = res || {};
      this.formatRawData();
    });
